...
- 前端请求统一放在 `web/src/services/api/`；AI 接口由浏览器直连，账号、画布、素材、生成记录与文件归属通过 `/api` 请求 `server/` 中的 Hono 后端。
...
- 当前版本必须登录；画布、素材和生成记录由 PostgreSQL 与私有 S3 共同持久化，浏览器只保留按账号隔离的草稿和媒体缓存。云端改造的实际部署、跨设备及浏览器验收状态以 pending-test 文档为准。
...
- 登录确认前不得加载个人 store；业务请求必须冻结发起账号并携带 `X-Session-User`，退出和切换账号时中止请求并重新初始化页面。
- AI 密钥和本地 Agent 历史不得写入云端业务数据；文件只保存稳定 `storageKey`，所有业务写入必须验证文件已完成上传且属于当前用户。
- 业务更新、删除必须携带 revision；冲突或网络失败保留账号草稿，手动重试，不自动覆盖云端或循环重试。
- 文件物理删除只能通过管理员清理命令：先预览，执行时锁定用户并提交 deleting 状态后删除 S3 对象，失败可重跑；不得因浏览器当前未加载某画布而删除文件。
- PostgreSQL 与 S3 必须共同备份。会话固定 7 天、密码 8–128 字符、登录每 IP 每分钟 5 次、签名地址 15 分钟、单文件默认 100 MiB；调整这些边界前仍须用户确认。
- 批量编辑脚本先确认工作目录；在 server 子目录运行命令时，不得直接使用以仓库根目录为基准的相对文件路径。
